'''
A binary search tree are a data structure that maps keys to values.
Instead of worrying about the exact placement of the items in the
tree, we instead rely on the binary tree structure to provide
efficient searching.

The binary search tree relies on a simple, yet fundamental premise:
for each parent node, the left child holds a value smaller than the
parent, though the right child holds a larger value. In other words,
the parent holds a value larger than that of the left child, but
smaller than that of the right child.
'''
